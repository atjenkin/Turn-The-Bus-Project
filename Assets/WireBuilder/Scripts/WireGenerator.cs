    ...
    public class WireGenerator
    {
        ...
        private static void ValidateComponents(Wire wire)
        {
            ...
            if (wire.wireType.geometryType == WireType.GeometryType.Mesh)
            {
                ...
                //Add a mesh collider
                MeshCollider collider = wire.GetComponent<MeshCollider>();
                if (collider == false) collider = wire.gameObject.AddComponent<MeshCollider>();
                collider.sharedMesh = wire.meshFilter.sharedMesh;
            }
        }
